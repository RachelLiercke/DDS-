...
combined%>%filter(!is.na(IBU))%>%
  ...
  ggplot(aes(x=State, y = median, fill=State)) + geom_col() +
  ggtitle("Median International Bitterness Units of Beer by State") + ylab("Median IBU") +
  theme(legend.position = "none") 
...
combined2 = combined1 %>% filter(str_detect(combined1$Style, "IPA"))
IPAs = combined2

#Create column labeled IPA and if it is IPA make it = 1 and just regular ale = 0
